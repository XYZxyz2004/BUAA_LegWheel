  ...
  if(HAL_FDCAN_GetRxMessage(hfdcan,FDCAN_RX_FIFO0, &FDCan_Export_Data.fdcan_RxHeader, FDCan_Export_Data.FDCANx_Export_RxMessage)==HAL_OK)
	{
		...
		if( FDCan_Export_Data.fdcan_RxHeader.Identifier >= M1505B_READID_START  && FDCan_Export_Data.fdcan_RxHeader.Identifier <= M1505B_READID_END  )
		{
		M1505B_FUN.M1505B_getInfo(FDCan_Export_Data);
		}
