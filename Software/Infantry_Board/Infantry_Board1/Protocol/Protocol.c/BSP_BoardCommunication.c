...
void Board1_To_2(void)
{
    ...
    data2_Fun[2] = ControlMes.shoot_Speed;            //射频
    data2_Fun[3] |= (uint8_t)(ControlMes.fric_Flag & 0x01) <<0;
		...
		data2_Fun[4] = M2006_Array[Dial_Motor].realTorque >> 8;
		data2_Fun[5] = M2006_Array[Dial_Motor].realTorque ;
    Fdcan_Fun.FDCAN_SendData(FDCAN_SendHandle, &hfdcan2, FDCAN_STANDARD_ID, FDCAN_ID_GIMBAL , data2_Fun);
}
...
void Board1_getGimbalInfo(Fdcan_Export_Data_t RxMessage)
{
    ...
    ControlMes.Speed_Bullet /= 1000;
		ControlMes.tnndcolor  		= (uint8_t)(RxMessage.FDCANx_Export_RxMessage[6]>>0)&0x01;
