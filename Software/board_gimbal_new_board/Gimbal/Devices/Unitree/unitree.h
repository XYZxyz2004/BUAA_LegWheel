...
typedef struct MasterComd{      // 以 4个字节一组排列 ，不然编译器会凑整
                      // 定义 数据
  uint8_t mode;       // 关节模式选择
...
typedef struct {
  ...
  float K_P;  // 关节刚度系数
  float K_W;  // 关节速度系数
  COMData32 Res;  // 通讯保留字节  用于实现别的一些通讯内容
} MOTOR_send;

typedef struct {
	ServoComdDataV3 ServoData;
  // 定义 接收数据
...
extern ServoComdDataV3 temprecvl;
extern ServoComdDataV3 temprecvr;
