...
float Derta_Yaw;
/****************Pitch限位*****************/
const float Cloud_Pitch_Min = 2900.0;//*步兵*
const float Cloud_Pitch_Max = 4280.0;
int16_t Cloud_Pitch_level1 = 3614;
int16_t offseta;
const float Cloud_Pitch_Center = (Cloud_Pitch_Min + Cloud_Pitch_Max) * 0.5f;
...
{
    //保存启动时刻的机械角度
    Cloud.Target_Pitch = Cloud.Pitch_Raw = Cloud_Pitch_level ;;
	  Cloud_Init_Angle = Cloud_Pitch_level;
    ...
    One_Kalman_Create(&Cloud_PitchCurrent_Kalman, 7,40 );
...
void Cloud_Pitch_Angle_Set(void)
{
	...
	{
//		 /**********自瞄限位**********/
//		if (Cloud.AutoAim_Pitch > 15*22.75556)
//		{
//			 Cloud.AutoAim_Pitch = 15*22.75556 ;
//		}
//		else if (Cloud.AutoAim_Pitch < -15*22.75556)
//		{
//			 Cloud.AutoAim_Pitch = -15*22.75556 ;
//		}
		AutoAim_Cloud_Delta += (float)ControlMes.pitch_velocity * 0.006f;
		Cloud.Target_Pitch = M6020s_Pitch.realAngle + Cloud.AutoAim_Pitch + AutoAim_Cloud_Delta +offseta;//+ ControlMes.Gimbal_Chassis_Pitch_Angle * SEN_Saber_Pitch ;
		Cloud_Target_Aim_Flag = Cloud.Target_Pitch;
		Aim_Flag = 1;
	}
	else
	{
		Gimbal_Cloud_Delta += (float)ControlMes.pitch_velocity * 0.006f;
		Pitch_Conpensate_Angle = + ControlMes.Gimbal_Chassis_Pitch_Angle * SEN_Saber_Pitch;
		
		Cloud.Target_Pitch = Cloud_Init_Angle + Gimbal_Cloud_Delta ;//+ ControlMes.Gimbal_Chassis_Pitch_Angle * SEN_Saber_Pitch;
	...
	else if (Gimbal_Cloud_Delta < Cloud_Pitch_Min - Cloud_Init_Angle)
	{
	 ...
	 /**********Cloud_Delta限位**********/
	if (Cloud.AutoAim_Pitch + AutoAim_Cloud_Delta > Cloud_Pitch_Max - Cloud_Init_Angle)
	{
		 AutoAim_Cloud_Delta = Cloud_Pitch_Max - Cloud_Init_Angle ;
	}
	else if (Cloud.AutoAim_Pitch + AutoAim_Cloud_Delta < Cloud_Pitch_Min - Cloud_Init_Angle)
	{
		 AutoAim_Cloud_Delta = Cloud_Pitch_Min - Cloud_Init_Angle ;
	}
...
//    Cloud.Target_Pitch = Cloud_Pitch_level1;
    ...
    /*角度差值滤波*/
//		float alpha = fabs(Angle_Err_Pitch) > 100.0f ? low_pass_filter_sen1 : low_pass_filter_sen2;  // 滤波系数，根据实际情况调整
//		Angle_Err_Pitch = low_pass_filter(Angle_Err_Pitch, filtered_angle_err_pitch, alpha);
//		filtered_angle_err_pitch = Angle_Err_Pitch;  // 更新上一次滤波后的值
...
void Cloud_Sport_Out(void)
{
    ...
    /**********电流参数的设置**********/
		//if(M6020s_Pitch.InfoUpdateFlag == 1)
		//{
