...
void limit_pid_params(float Kp, float Ki, float Kd) 
{
    ...
    if (Kp < 0.1f) Kp = 0.1f;
    if (Kp > 40.0f) Kp = 40.0f;
    if (Ki < 0.001f) Ki = 0.001f;
    if (Ki > 0.5f) Ki = 0.5f;
    if (Kd < 0.001f) Kd = 0.001f;
    if (Kd > 30.f) Kd = 30.f;
}
...
float PID_Model4_Update(incrementalpid_t *pid,FUZZYPID_Data_t *PID, float _set_point, float _now_point)
{
    ...
    pid->error_target = pid->Target - pid->last_set_point;
	  delta_error = pid->err-pid->err_last;
		...
		fuzzy_kf=pid->kf;
//	  // 神经网络输入
//    float input[INPUT_SIZE] = {pid->err, delta_error};
//    
//    // 神经网络输出（预测的PID参数）
...
//    network_ki = output[1];
//    network_kd = output[2];
//		limit_pid_params(network_kp,network_ki,network_kd);
...
float Incremental_PID_New(Struct_PID_Manage_Object_Fuzzy *pid_t, float target, float measured)
{

    pid_t->set_point  = target;
    pid_t->now_point = measured;
    pid_t->error = pid_t->set_point - pid_t->now_point;
    ...
    pid_t->d_out = pid_t->kd * (pid_t->error - 2.0f * pid_t->last_error + pid_t->before_last_error);

    //积分限幅
    abs_limit(&pid_t->i_out, pid_t->integral_limit);

    pid_t->output += (pid_t->p_out + pid_t->i_out + pid_t->d_out);

    //输出限幅
    abs_limit(&pid_t->output, pid_t->output_limit);

    pid_t->before_last_error = pid_t->last_error;
...
void Incremental_PIDInit_New(Struct_PID_Manage_Object_Fuzzy *pid_t, float Kp, float Ki, float Kd, uint32_t MaxOutput, uint32_t IntegralLimit)
{
    pid_t->kp = Kp;
    pid_t->ki = Ki;
    pid_t->kd = Kd;
...
float Position_PID_New(Struct_PID_Manage_Object_Fuzzy *pid_t, float target, float measured)
{

    pid_t->set_point = (float)target;
    pid_t->now_point = (float)measured;
    pid_t->error = pid_t->set_point - pid_t->now_point;
    pid_t->err_change = pid_t->error - pid_t->last_error;

    pid_t->p_out = pid_t->kp * pid_t->error;
    pid_t->i_out += pid_t->ki * pid_t->error;
    pid_t->d_out = pid_t->kd * (pid_t->error - pid_t->last_error);
    //积分限幅
    abs_limit(&pid_t->i_out, pid_t->integral_limit); //取消积分输出的限幅。
...
void Position_PIDInit_New(Struct_PID_Manage_Object_Fuzzy *pid_t, float Kp, float Ki, float Kd, float MaxOutput, float Integral_Separation, float IntegralLimit)
{
	  pid_t->kp = Kp;
    pid_t->ki = Ki;
    pid_t->kd = Kd;
