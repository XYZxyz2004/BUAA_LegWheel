...
//			}
			if(RC_CtrlData.rc.s2 == RC_SW_MID)
			{
		...
		else if(RC_CtrlData.rc.s1 == RC_SW_DOWN && RC_CtrlData.rc.s2 != 0)
		{
			...
			ControlMes.x_velocity =  -RC_CtrlData.rc.ch3; //左手上下
			ControlMes.y_velocity =  -RC_CtrlData.rc.ch2; //左手左右
//			if(ControlMes.x_velocity >= 650)
//			{
//				ControlMes.feipo_Flag = 1;
//			}
//			else
//			{
//				ControlMes.feipo_Flag = 0;
//			}
			...
			{
				//Dial_Data.Shoot_Interval = 330 - abs(RC_CtrlData.wheel) / 2;
			  Dial_Data.Speed_Dial = speed +(RC_CtrlData.wheel) * 5;
//				Dial_Data.Dial_Angle -= kk;
//				if(Dial_Data.Dial_Angle >8192)
//				{
//					Dial_Data.Dial_Angle-=8192;
//				}
//				else if(Dial_Data.Dial_Angle <0)
//				{
//					Dial_Data.Dial_Angle+=8192;
//				}
				ControlMes.modelFlag = model_Normal;
...
//				}
		...
		/*发射信息处理*/
		if(( ControlMes.shoot_state == RC_SW_DOWN||RC_CtrlData.rc.s2 == RC_SW_UP)&&RC_CtrlData.rc.s1 != RC_SW_MID)
		{
			Dial_Data.Shoot_Mode = Continuous_Shoot;
			Shoot_Data.Shoot_Switch = 1;
		}
		else if( ControlMes.shoot_state == RC_SW_DOWN&&RC_CtrlData.rc.s1 == RC_SW_MID)
		{
			Dial_Data.Shoot_Mode = Continuous_Shoot;
			Shoot_Data.Shoot_Switch = 1;
		}
		else if(ControlMes.shoot_state == RC_SW_MID)
		{
			...
			//Dial_Data.Shoot_Interval = 100;
		}
		//ControlMes.z_rotation_velocity = RC_CtrlData.rc.ch1; // 右手上下
		//ControlMes.gimbal_angle =RC_CtrlData.wheel;
...
static int RampRate_ForwardBack = 30;              //斜坡函数叠加值
static int RampRate_LeftRight = 30;              //斜坡函数叠加值
...
void RemoteControl_PC_Update(void)
{
	...
	//云台控制
	ControlMes.yaw_velocity = RC_CtrlData.mouse.x * 6 ;
	ControlMes.pitch_velocity = -RC_CtrlData.mouse.y * 4 ;
